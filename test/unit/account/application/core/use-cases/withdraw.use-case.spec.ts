...
describe('WithdrawUseCase', () => { 
    ...
    it('Should throw an error if account does not have enough balance', async () => {
        ...
        await expect(withdrawUseCase.execute(event)).rejects.toThrow('Insufficient funds in account: 1');
    ...
    it('Should throw an error if account not found', async () => {
        ...
        await expect(withdrawUseCase.execute(event)).rejects.toThrow('Origin account not found: 1');
